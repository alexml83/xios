...
unsigned int nStakeMinAge = 1 * 60; // 1 hour
...
const string strMessageMagic = "XIOS Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (nHeight > 2 && nHeight < 1441)
		{
			nSubsidy = 1333 * COIN;
		}
	else if (nHeight > 1440 && nHeight < 3441)
		{
			nSubsidy = 33.3 * COIN;
		}
	else if (nHeight > 3440 && nHeight < 9442)
		{
			nSubsidy = 3.3 * COIN;
		}	
	else if (nHeight > 9441 && nHeight < 500001) 
		{
			nSubsidy = 1.33 * COIN;
		}
	else
...
static int64_t nTargetTimespan = 4 * 60;  // 10 mins
        ...
        if(MasternodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    CXIOScoinAddress address2(address1);
